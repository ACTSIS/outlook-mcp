...
const startServer = (done) => {
  ...
  jest.isolateModules(() => {
    ...
    if (!sseApp) {
      // Create a dummy app if not loaded (which it won't be)
      console.warn(
        'sse-server.test.js: Using a DUMMY app for SSE tests. sse-server.js needs to be refactored for proper testing.'
      );
      try {
...
describe('SSE Server (sse-server.js)', () => {
  ...
  it('KNOWN ISSUE: sse-server.js structure makes direct app testing difficult', () => {
    ...
    try {
      ...
      /* ignore if file not found for some reason */
    }

    const directlyListens = /app\.listen\s*\(/m.test(sseServerFileContent);
    if (directlyListens) {
      console.warn(
        'sse-server.test.js: WARNING - sse-server.js appears to call app.listen() directly. This makes it hard to test. Consider refactoring to export the app instance and listen in a separate executable file.'
      );
    }
