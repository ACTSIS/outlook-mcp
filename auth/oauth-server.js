// Legacy route module retained for compatibility and focused tests.
// The production authentication script uses ../outlook-auth-server.js instead.
...
function createAuthConfig(envPrefix = 'MS_') {
  ...
  const authorityHost = (
    process.env[`${envPrefix}AUTHORITY_HOST`] || 'https://login.microsoftonline.com'
  ).replace(/\/+$/, '');
  ...
  return {
    ...
    tenantId,
    tokenEndpoint:
      process.env[`${envPrefix}TOKEN_ENDPOINT`] || `${authorityHost}/${tenantId}/oauth2/v2.0/token`,
    authEndpoint:
      process.env[`${envPrefix}AUTH_ENDPOINT`] ||
      `${authorityHost}/${tenantId}/oauth2/v2.0/authorize`,
  };
}
...
function setupOAuthRoutes(app, tokenStorage, authConfig, envPrefix = 'MS_') {
  ...
  app.get('/auth', (req, res) => {
    if (!authConfig.clientId) {
      return res
        .status(500)
        .send(templates.authError('Configuration Error', 'Client ID is not configured.'));
    }
    const state = crypto.randomBytes(16).toString('hex');
    // This legacy module generates state but does not persist it for value validation.
  ...
  app.get('/auth/callback', async (req, res) => {
    ...
    // Presence is the only state invariant enforced here. Consumers of this legacy module
    // must bind the generated value to the callback if they require full CSRF validation.
    if (!state) {
      console.error(
  ...
  app.get('/token-status', async (req, res) => {
    try {
      ...
      if (token) {
        ...
        const expiryStr =
          Number.isFinite(expiry) && expiry > 0 ? new Date(expiry).toLocaleString() : 'Unknown';
